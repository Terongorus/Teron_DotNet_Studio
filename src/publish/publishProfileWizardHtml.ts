...
/**
 * 2-step wizard mirroring this extension's own Create New Project wizard (newProjectHtml.ts):
 * step 1 picks the publish target type (a card per type, with a description, since there are only
 * 5 fixed choices rather than a searchable list), step 2 configures that profile's details. Same
 * step-indicator/step-panel/footer mechanics, same one-shot-dispose-on-success panel lifecycle
 * (see publishProfileWizardPanel.ts).
 */
export function getPublishProfileWizardHtml(webview: vscode.Webview, codiconCssUri: vscode.Uri, projectName: string, runtimeIdentifiers: string[]): string {
...
<head>
    ...
    <title>Publish Profile: ${projectName}</title>
    <style nonce="${nonce}">
        body {
...
<body>
    <h1 id="pageTitle">New Publish Profile</h1>
    ...
    <script nonce="${nonce}">
        ...
        saveBtn.addEventListener('click', () => {
            ...
            vscode.postMessage({ command: 'save', profile: collectProfile(), secret: collectSecret(), isEditing });
        });
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.command) {
                ...
                case 'editProfile': {
                    // Pre-populates every field from an existing profile and jumps straight to
                    // step 2 - Back still works if the user wants to change target type.
                    isEditing = true;
                    pageTitle.textContent = 'Edit Publish Profile';
                    const p = message.profile;
                    selectTargetType(p.targetType);
                    profileNameEl.value = p.name;
                    profileNameEl.disabled = true; // renaming happens from the Publish page, not here
