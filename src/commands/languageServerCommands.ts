...
import { getBundledCommand } from '../languageServer/sharpLspLocator';
...
export function registerLanguageServerCommands(context: vscode.ExtensionContext, manager: SharpLspClientManager): void {
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('dotnet-creator.sharpLsp.showMenu', () => showMenu(context, manager))
    );
}

async function showMenu(context: vscode.ExtensionContext, manager: SharpLspClientManager): Promise<void> {
    type Item = vscode.QuickPickItem & { action: 'restart' | 'showOutput' | 'bundled' | 'download' | 'instructions' | 'openSettings' };
    ...
    if (getBundledCommand(context)) {
        items.push({ label: '$(archive) Use Bundled SharpLsp', action: 'bundled' });
    }
    items.push(
