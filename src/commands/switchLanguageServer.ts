...
/**
 * Mutually exclusive by design: dotnet-creator.languageServer selects exactly one, and only that
...
export function registerSwitchLanguageServerCommand(context: vscode.ExtensionContext, sharpLsp: SharpLspClientManager, roslyn: RoslynClientManager): void {
    context.subscriptions.push(
        vscode.commands.registerCommand('dotnet-creator.switchLanguageServer', () => switchLanguageServer(sharpLsp, roslyn))
    );
}

async function switchLanguageServer(sharpLsp: SharpLspClientManager, roslyn: RoslynClientManager): Promise<void> {
    const config = vscode.workspace.getConfiguration('dotnet-creator');
