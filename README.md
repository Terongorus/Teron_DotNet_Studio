...
* **Self-Update via GitHub Releases:** .NET Studio checks its own [GitHub releases](https://github.com/Terongorus/Teron_DotNet_Studio/releases) (not the VS Code Marketplace) once a day and offers to download and install the latest `.vsix` directly — **.NET: Check for Updates** triggers this on demand. GitHub Releases is this extension's actual distribution channel going forward.
