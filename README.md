...
* **Publish:** an interactive Publish panel (**Publish...** on a project's Solution Explorer context menu, or from the Project status bar item) for creating and managing folder publish profiles — target framework, framework-dependent vs. self-contained, target runtime, target location, and the single-file/ReadyToRun/trimming options that make sense for the selected mode — including compressing the single-file bundle and the legacy "include all content" option, both gated to when they'd actually do anything (single-file, and self-contained for compression specifically - the .NET SDK raises a real build error otherwise). Profiles are saved as real `Properties/PublishProfiles/<name>.pubxml` files using Visual Studio's own schema, so they're fully interoperable with VS's own Publish UI — this doesn't invent a parallel profile format. Clicking **Publish** restores explicitly (with the profile's own `Configuration`/`RuntimeIdentifier`, so a self-contained/RID-specific publish always gets the right restored assets) and then runs `dotnet publish` against the saved profile in a visible terminal. Folder targets only for now — no Azure/IIS/container publish.
